...
const Reward = ({ reward, onClick, id }: IReward) => {
  const preventClick = () => {
    ...
    >
      ...
      >
        ...
        <div className="absolute -top-3 left-0 flex w-full justify-center items-center">
          <span className="bg-red-500 rounded text-xs px-1 py-0.5 text-white">Disabled</span>
        </div>
        )}
      </div>
      <h2 className="text-white text-xs mt-1">{reward.title}</h2>
      <h3 className="text-white mt-1" style={{ fontSize: '0.5rem' }}>{id}</h3>
    </div>
  );
};
